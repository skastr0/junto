...
/**
 * Evaluate verb grant compatibility between candidate compiled grants and domain grants.
 */
export const evaluateVerbGrantCompatibility = (
  candidate: VerbGrant,
  current: VerbGrant,
): SemanticCompatibilityEvaluation<VerbGrant> => {
  const candidatePorts = candidate.ports ?? [];
  const currentPorts = current.ports ?? [];

  const grantEval = evaluateGrantCompatibility(candidatePorts, currentPorts);
  if (grantEval.status === "unsupported") {
    return {
      status: "unsupported",
      reasonCode: grantEval.reasonCode,
      message: grantEval.message,
      differences: grantEval.differences,
    };
  }

  // Check assignable flag (must not widen assignable from false to true)
  if (candidate.assignable === true && current.assignable !== true) {
    return {
      status: "unsupported",
      reasonCode: "authority-widening",
      message: "Candidate grants assignable capability not present in current domain grant",
      differences: [
        {
          aspect: "grant",
          detail: "Unauthorized assignable grant",
        },
      ],
    };
  }

  // Check wake flag (must not widen wake)
  if (candidate.wake === true && current.wake !== true) {
    return {
      status: "unsupported",
      reasonCode: "authority-widening",
      message: "Candidate grants wake capability not present in current domain grant",
      differences: [
        {
          aspect: "grant",
          detail: "Unauthorized wake grant",
        },
      ],
    };
  }

  // Check flow flag (must not widen flow)
  if (candidate.flow === true && current.flow !== true) {
    return {
      status: "unsupported",
      reasonCode: "authority-widening",
      message: "Candidate grants flow capability not present in current domain grant",
      differences: [
        {
          aspect: "grant",
          detail: "Unauthorized flow grant",
        },
      ],
    };
  }
  ...
  // Check chain flag
  if (candidate.chain === true && current.chain !== true) {
    return {
      status: "unsupported",
      reasonCode: "authority-widening",
      message: "Candidate grants chain capability not present in current domain grant",
      differences: [
        {
          aspect: "grant",
          detail: "Unauthorized chain grant",
        },
      ],
    };
  }

  // If exact ports and exact flags
  const exactFlags =
    Boolean(candidate.assignable) === Boolean(current.assignable) &&
    Boolean(candidate.wake) === Boolean(current.wake) &&
    Boolean(candidate.flow) === Boolean(current.flow) &&
    Boolean(candidate.chain) === Boolean(current.chain);

  if (grantEval.status === "exact" && exactFlags) {
    return {
      status: "exact",
      value: candidate,
    };
  ...
  // Restricted: candidate is narrower
  const withheld: SemanticDifference[] = [...(grantEval.status === "restricted" ? grantEval.withheldSemantics : [])];
  if (current.assignable && !candidate.assignable) {
    withheld.push({ aspect: "grant", detail: "Assignable capability withheld" });
  }
  if (current.wake && !candidate.wake) {
    withheld.push({ aspect: "grant", detail: "Wake capability withheld" });
  }
  if (current.flow && !candidate.flow) {
    withheld.push({ aspect: "grant", detail: "Flow capability withheld" });
  }
  if (current.chain && !candidate.chain) {
    withheld.push({ aspect: "grant", detail: "Chain capability withheld" });
  }
