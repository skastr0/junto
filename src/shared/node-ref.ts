// Canonical, locator-only references to nodes in Vellum canvas documents.
...
export const parseNodeRef = (input: string): NodeRefParseResult => {
  if (
    ...
    return fail("shape", "Vellum reference is empty, oversized, or contains control characters");
  }

  const schemeEnd = input.indexOf("://");
  if (schemeEnd < 0 || input.slice(0, schemeEnd) !== SCHEME) {
    return fail("scheme", "Vellum reference must use the lowercase vellum scheme");
  }
  ...
  if (authorityEnd < 0 || afterScheme.slice(0, authorityEnd) !== AUTHORITY) {
    return fail("authority", "Vellum reference authority must be exactly canvas");
  }
  if (input.includes("#")) {
    return fail("shape", "Vellum references do not accept fragments");
  }

  const pathAndQuery = afterScheme.slice(authorityEnd + 1);
  const queryStart = pathAndQuery.indexOf("?");
  if (queryStart < 0 || pathAndQuery.indexOf("?", queryStart + 1) >= 0) {
    return fail("shape", "Vellum reference must contain exactly one node query");
  }
  ...
  if (canvasName.includes("/") || query.includes("&") || !query.startsWith("node=")) {
    return fail("shape", "Vellum reference must be /<canvas-name>?node=<node-id>");
  }
  ...
  if (canonical !== input) {
    return fail("canonical", "Vellum reference is not in canonical form", canonical);
  }
