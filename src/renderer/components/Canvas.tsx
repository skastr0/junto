...
import { Expand, FileText, Link2, Plus, ScanLine, SquareDashed } from "lucide-react";
...
import { makeFileNode, makeGroupNode, makeLinkNode, makeTextNode } from "../lib/node-factories";
...
function useCanvasDocument(docVersion: number, searchQuery: string, edgeFilter: EtherEdgeKind | "", sourceFilter: EntitySource | "", flagFilter: EtherFlag | "", selectedNodeId: string, selectedEdgeId: string, setNodes: ReturnType<typeof useNodesState<FlowNode>>[1], setEdges: ReturnType<typeof useEdgesState<FlowEdge>>[1]) {
  useEffect(() => {
    const built = toFlow(state$.doc.peek());
    const sourceNodes = sourceFilter ? built.nodes.filter((node) => node.type === "group" || node.data?.node.ether?.bindings?.some((binding) => binding.source === sourceFilter)) : built.nodes;
    const visibleNodes = flagFilter ? sourceNodes.filter((node) => node.type === "group" || node.data?.node.ether?.flags?.includes(flagFilter)) : sourceNodes;
  ...
  }, [docVersion, edgeFilter, flagFilter, searchQuery, sourceFilter, selectedNodeId, selectedEdgeId, setNodes, setEdges]);
}
...
function useCanvasFocus(focusNodeId: string, rf: CanvasFlow) {
  useEffect(() => {
    ...
    const focus = () => {
      ...
      // React Flow emits an empty selection while the field mounts. Re-apply
      // the manifest target only after it is present in the live graph.
...
function useCanvasViewport(canvasName: string, nodeCount: number, rf: CanvasFlow) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      // if the first frame fits every signal. Regions are the spatial index;
      // when none exist, show the first signal cluster.
      fitReadableField(rf);
    });
    return () => cancelAnimationFrame(frame);
...
function useCanvasInteractions(rf: CanvasFlow) {
  ...
  const onSelectionChange = useCallback(({ nodes: selectedNodes, edges: selectedEdges }: { readonly nodes: ReadonlyArray<FlowNode>; readonly edges: ReadonlyArray<FlowEdge> }) => {
    ...
    // click is the explicit deselection gesture; do not erase a manifest or
    // inspector selection from an internal remount event.
...
function AddNodePanel() {
  ...
  const create = (kind: "text" | "file" | "link" | "group") => {
    ...
    const position = gridPlacement ? gridPlacement : findOpenPosition(docNodes, center, size);
  ...
  return (
    <Panel position="top-left" className="node-palette-panel">
      <div className="node-palette">
        <button className="node-palette__trigger" aria-label="Add canvas item" aria-expanded={open} onClick={() => setOpen((value) => !value)}><Plus size={14} /><span>add item</span></button>
        {open ? <div className="node-palette__menu">
...
function FitAllPanel() {
  const rf = useReactFlow<FlowNode, FlowEdge>();
  return <Panel position="top-center" className="field-fit-panel"><button type="button" aria-label="Fit all signals" title="fit all signals" onClick={() => void rf.fitView({ padding: 0.18, duration: 320, maxZoom: 1.35 })}><Expand size={12} />fit all</button></Panel>;
}
...
function useCanvasGraph() {
  ...
  const rf = useReactFlow<FlowNode, FlowEdge>();
  useCanvasDocument(docVersion, searchQuery, edgeFilter, sourceFilter, flagFilter, selectedNodeId, selectedEdgeId, setNodes, setEdges);
  useCanvasSearchViewport(searchQuery, nodes.length, rf, `${edgeFilter}|${sourceFilter}|${flagFilter}`);
