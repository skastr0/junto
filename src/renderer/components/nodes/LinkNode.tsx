        ...
        <div className="flex h-full w-full flex-col justify-between overflow-hidden">
          <button type="button" className="nopan flex w-full items-start gap-2 border-0 bg-transparent p-0 text-left" onClick={(event) => { if (!selected) return; event.stopPropagation(); setEditing(true); }} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setEditing(true); }}>
            ...
            <div className="min-w-0">
              <div
                className="truncate font-mono text-[12px] font-semibold underline decoration-dotted underline-offset-2"
                style={{ color: INK }}
              >
                {host}
              </div>
          ...
          <button
            type="button"
            className="nodrag nopan mt-1 flex w-fit items-center gap-1 rounded border border-white/10 px-1.5 py-0.5 text-[9px] text-ink-2 hover:bg-white/10"
