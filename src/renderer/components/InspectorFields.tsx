...
export function ConnectSignalEditor({ node, doc, open, onOpenChange }: { readonly node: CanvasNode; readonly doc: CanvasDoc; readonly open: boolean; readonly onOpenChange: (open: boolean) => void }) {
  ...
  if (!open) return null;
  return <div className="inspector-connect"><label><span>filter targets <em>{filteredTargets.length}/{availableTargets.length}</em></span><input aria-label="Filter targets" value={targetQuery} onChange={(event) => setTargetQuery(event.target.value)} placeholder="name, source, or binding" /></label>{availableTargets.length === 0 ? <div className="inspector-connect__empty">No unlinked signals available.</div> : filteredTargets.length === 0 ? <div className="inspector-connect__empty">No signals match this filter.</div> : <label><span>target signal</span><select aria-label="Target signal" value={targetId} onChange={(event) => setTargetId(event.target.value)}><option value="">choose a signal</option>{filteredTargets.map((target) => <option key={target.id} value={target.id}>{nodeTitle(target)} · {target.type}</option>)}</select></label>}<label><span>relation kind</span><select aria-label="Relation kind" value={edgeKind} onChange={(event) => setEdgeKind(event.target.value as EtherEdgeKind)}><option value="relates">relates</option><option value="depends">depends</option><option value="blocks">blocks</option></select></label><button disabled={!targetId} onClick={connect}>create relation</button></div>;
}
