 ...
 * viewport allows: right of the rect, then below, left, above. Each side
 * hugs the rect's bottom-right corner. With no clear side, the menu sits at
 * that corner, clamped into the viewport.
 */
...
export const placeBesideRect = (
  ...
  const candidates = [
    { x: rect.right + GAP, y: alignedY },
    { x: alignedX, y: rect.bottom + GAP },
    { x: rect.left - GAP - menu.width, y: alignedY },
    { x: alignedX, y: rect.top - GAP - menu.height },
  ];
  const fits = candidates.find((point) =>
