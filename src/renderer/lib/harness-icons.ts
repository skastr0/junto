...
//                ones (brand color or source art the CodexBar set lacks).
...
// replace them with generated data: claude/gemini carry brand color, openai
// is the Wikimedia symbol on its native 20×20 grid, and the CodexBar set
// skips kimi and windsurf entirely. Everything else (including grok, codex,
// devin, amp — same source data) resolves from the generated table.
...
export function markTileFor(agent: string | undefined, service: MarksService = marks): MarkTile {
  ...
  return {
    ...
    paths: glyph ? (Array.isArray(glyph.d) ? glyph.d : [glyph.d]) : [],
