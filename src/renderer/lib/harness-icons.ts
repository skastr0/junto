import { HUE, INK } from "./theme";

// Brand glyph data for agent harnesses (claude, kimi, codex, …). Path data is
// inlined from the brand SVGs — the renderer has no asset-loader convention.
// Most marks are a single path on a 24×24 viewBox; marks whose source art is
// multi-path or on another grid carry a `d` array and an explicit `viewBox`.
// Brand hex is the source-of-truth mark color; near-black brands are remapped
// to house INK by harnessHue so they stay visible on the dark field.
...
export const GLYPHS: Readonly<Record<string, HarnessGlyph>> = {
  ...
  grok: {
    // xAI monogram — source art is 4 polygons on an 841.89×595.28 grid; the
    // viewBox crops to the mark itself.
    d: [
      "M557.09,211.99 L565.4,538.36 L631.96,538.36 L640.28,93.18 Z",
      "M640.28,56.91 L538.72,56.91 L379.35,284.53 L430.13,357.05 Z",
      "M201.61,538.36 L303.17,538.36 L353.96,465.84 L303.17,393.31 Z",
      "M201.61,211.99 L430.13,538.36 L531.69,538.36 L303.17,211.99 Z",
    ],
    viewBox: "190 50 460 500",
    hex: "#000000",
    displayName: "Grok",
  },
  openai: {
...
const ALIASES: Readonly<Record<string, string>> = {
  ...
  // CodexBar provider ids (and common CLI spellings) → brand glyph keys.
  codex: "openai",
  chatgpt: "openai",
  "chat gpt": "openai",
  gpt: "openai",
