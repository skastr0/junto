...
// returns a derived verdict. lib/kernel-state.ts owns the loop that keeps
...
// lib/browse.ts's cached towerBrowse fetch. A project absent from the index
...
// Keyed by watcher NODE id, not by the watch rule's content, so two distinct
// watcher nodes that happen to describe the same rule still get independent
// baselines and independent fire history.
const seenLevelStatus = new Map<string, WatcherStatus>();
// Keyed by `${watcherId}::${orbit}::${glyphId}` so per-glyph baselines are
// scoped to the watcher that is watching them.
...
const evaluateGlyphsEnteredState = (
  ...
  for (const glyph of scoped) {
    const memoryKey = `${watcherId}::${glyph.orbit}::${glyph.glyphId}`;
...
export function evaluateWatcher(
  ...
  if (watch.kind === "glyphs_entered_state") {
    const { evaluation, firedGlyphIds } = evaluateGlyphsEnteredState(watcherId, watch, glyphIndex);
...
export function detectPulses(
  ...
  for (const node of doc.nodes) {
    ...
    if (!watch) continue;
    const result = evaluateWatcher(node.id, watch, snapshots, glyphIndex);
