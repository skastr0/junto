...
// ---------------------------------------------------------------------------
// Station projection frame delivery (under remote ~/.vellum/projections/)
// ---------------------------------------------------------------------------

/**
 * Fixed relative drop path under remote home `.vellum`.
 ...
 * `<home>/.vellum/projections/incoming.frame`.
 */
export const confineProjectionIncomingPath = (
  remoteHome: string,
  ...
  const path = `${projectionsDir}/${PROJECTION_INCOMING_BASENAME}`;
  if (
...
/**
 * Compile the sole remote shell for Command Center → Station projection push:
 * umask → ensure `.vellum/projections` → exclusive temp stdin write → rename to
 * `incoming.frame` (atomic replace of the drop path).
 *
 * Beta ship: no packaged bridge binary — Remote polls/applies the drop file.
 * Paths are product-confined; transport must not hand-author shell strings.
 */
export const compileProjectionFrameDeliver = (
  remoteHome: string,
): Effect.Effect<
  { readonly command: RemoteCommand; readonly path: string },
  SshInputError
> =>
  confineProjectionIncomingPath(remoteHome).pipe(
    Effect.flatMap((path) => {
      const projectionsDir = path.slice(
        0,
        path.length - PROJECTION_INCOMING_BASENAME.length - 1,
      );
      const vellumDir = projectionsDir.slice(
        0,
        projectionsDir.length - PROJECTION_DROP_RELATIVE_DIR.length - 1,
      );
      const dir = shellSingleQuote(projectionsDir);
      const vellum = shellSingleQuote(vellumDir);
      const file = shellSingleQuote(path);
      // Fixed product suffix — never interpolate user data into shell words.
      const tmpBase = shellSingleQuote(`${path}.vellum-projection`);
      const source = [
        "set -eu",
        "umask 077",
        `if [ -L ${vellum} ]; then printf '%s\\n' 'vellum-remote-plan: .vellum is a symlink' >&2; exit 73; fi`,
        `/bin/mkdir -p -- ${vellum}`,
        `if [ -L ${vellum} ] || [ ! -d ${vellum} ]; then printf '%s\\n' 'vellum-remote-plan: .vellum unsafe' >&2; exit 73; fi`,
        `if [ -L ${dir} ]; then printf '%s\\n' 'vellum-remote-plan: projections dir is a symlink' >&2; exit 73; fi`,
        `/bin/mkdir -p -- ${dir}`,
        `if [ -L ${dir} ] || [ ! -d ${dir} ]; then printf '%s\\n' 'vellum-remote-plan: projections dir unsafe' >&2; exit 73; fi`,
        `if [ -L ${file} ]; then printf '%s\\n' 'vellum-remote-plan: incoming frame is a symlink' >&2; exit 73; fi`,
        `if [ -e ${file} ] && [ ! -f ${file} ]; then printf '%s\\n' 'vellum-remote-plan: incoming frame not a regular file' >&2; exit 73; fi`,
        // Exclusive create of temp via noclobber; $$ is remote shell pid only.
        `tmp=${tmpBase}.$$`,
        `if [ -e "$tmp" ] || [ -L "$tmp" ]; then printf '%s\\n' 'vellum-remote-plan: temp path busy' >&2; exit 73; fi`,
        "set -C",
        `cat > "$tmp" || { set +C; /bin/rm -f -- "$tmp"; exit 73; }`,
        "set +C",
        `/bin/chmod 600 -- "$tmp" || { /bin/rm -f -- "$tmp"; exit 73; }`,
        `/bin/mv -f -- "$tmp" ${file} || { /bin/rm -f -- "$tmp"; exit 73; }`,
        `/bin/chmod 600 -- ${file}`,
        "",
      ].join("\n");
      return makeRemoteCommand("/bin/sh", [
        "-c",
        source,
        "vellum-plan:projection-frame-deliver",
