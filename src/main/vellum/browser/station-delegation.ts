...
export const mintStationBrowserEnvelope = (witness: AdmittedDelegationWitness, request: Omit<StationBrowserRequest, "authority" | "originStationId" | "agentRef">, keyId: string, privateKey: KeyObject): StationBrowserEnvelope => { const data = witnesses.get(witness); if (data === undefined) throw new Error("station browser delegation requires a main-admitted witness"); if (!isStationBrowserKeyId(keyId) || privateKey.type !== "private" || privateKey.asymmetricKeyType !== "ed25519") throw new Error("station browser delegation requires an Ed25519 private key and canonical key id"); const bound = { ...request, authority: data.kind, originStationId: data.stationId, agentRef: data.agentRef ?? null, pageRef: request.pageRef ?? null, session: request.session ?? null, payload: request.payload ?? null } as unknown as StationBrowserRequest; return { request: bound, keyId, signature: sign(null, signed(bound), privateKey).toString("base64url") }; };
