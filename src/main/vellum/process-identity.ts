...
export interface UnixPeerProcessHop {
  readonly pid: number;
...
export interface UnixPeerProcessChain {
  readonly platform: "Darwin" | "Linux";
  readonly peerPid: number;
  readonly peerUid: number;
  readonly chain: readonly UnixPeerProcessHop[];
}

export type PeerProcessChainReader = (
  socket: Socket,
) => UnixPeerProcessChain | undefined;
...
const canonicalNonNegativeInteger = (value: unknown): value is string =>
  typeof value === "string" && /^(?:0|[1-9][0-9]*)$/u.test(value);

const canonicalPositiveInteger = (value: unknown): value is string =>
  typeof value === "string" && /^[1-9][0-9]*$/u.test(value);

const decodePeerProcessChain = (
  input: unknown,
): UnixPeerProcessChain | undefined => {
  if (
    typeof input !== "object" ||
    ...
    !("peer_pid" in input) ||
    !safePositiveInteger(input.peer_pid) ||
    !("peer_uid" in input) ||
    !safeNonNegativeInteger(input.peer_uid) ||
    !("chain" in input) ||
    !Array.isArray(input.chain) ||
    input.chain.length < 1 ||
    input.chain.length > 12
  ) {
    return undefined;
  }

  const chain: UnixPeerProcessHop[] = [];
  for (const candidate of input.chain) {
    if (
      typeof candidate !== "object" ||
      candidate === null ||
      !("pid" in candidate) ||
      !safePositiveInteger(candidate.pid) ||
      !("ppid" in candidate) ||
      !safeNonNegativeInteger(candidate.ppid) ||
      !("uid" in candidate) ||
      !safeNonNegativeInteger(candidate.uid) ||
      !("start_key" in candidate) ||
      typeof candidate.start_key !== "string" ||
      !/^[0-9]+(?::[0-9]+)?$/u.test(candidate.start_key) ||
      !("executable" in candidate) ||
      typeof candidate.executable !== "string" ||
      !isAbsolute(candidate.executable) ||
      candidate.executable.length > 4096 ||
      !("device" in candidate) ||
      !canonicalNonNegativeInteger(candidate.device) ||
      !("inode" in candidate) ||
      !canonicalPositiveInteger(candidate.inode)
    ) {
      return undefined;
    }
    chain.push({
      ...
      ppid: candidate.ppid,
      uid: candidate.uid,
      startKey: candidate.start_key,
      executable: normalize(candidate.executable),
      device: candidate.device,
      inode: candidate.inode,
    });
  }
  if (
    chain[0]?.pid !== input.peer_pid ||
    chain[0]?.uid !== input.peer_uid
  ) {
    return undefined;
  }
  ...
  return Object.freeze({
    ...
    peerPid: input.peer_pid,
    peerUid: input.peer_uid,
    chain: Object.freeze(chain.map((hop) => Object.freeze(hop))),
  });
};

/**
 * Capture one kernel-peer-bound process ancestry observation.
 *
 * The sealed helper resolves exact executable files through libproc (Darwin)
 * or procfs (Linux) and reads every pid/ppid/start tuple twice. Callers must
 * still apply domain policy and take a second observation before dispatch.
 */
export const readUnixPeerProcessChain: PeerProcessChainReader = (socket) => {
  ...
  try {
    const result = spawnSync(python, [helper, "--process-chain"], {
      stdio: [fd, "pipe", "pipe"],
      encoding: "utf8",
      timeout: 2_000,
      maxBuffer: 64 * 1024,
    ...
    return decodePeerProcessChain(JSON.parse(result.stdout) as unknown);
  } catch {
    return undefined;
  }
};
