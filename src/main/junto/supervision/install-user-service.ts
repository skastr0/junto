...
const RELEASE_WRAPPER_MARKER = "/resources/bin/junto-remote" as const;
/** Bundled Node entry (wrapper execs node on this path). */
const RELEASE_ENTRY_MARKER = "/resources/app-remote/junto-remote.js" as const;
const CLI_RELATIVE = "resources/bin/junto" as const;
const CLI_HELPER_RELATIVE = ".local/bin/junto" as const;
 ...
 * Staging holds `junto-runtime-<semver>-linux-x64` before activation.
 */
const CANDIDATE_RUNTIME_ROOT =
  /^\/(?:[^/\u0000-\u001f\u007f]+\/)*\.junto\/runtime\/(?:releases\/(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)-[0-9a-f]{64}|staging\/[^/\u0000-\u001f\u007f]+\/junto-runtime-(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)-linux-x64)$/u;
...
const resolveRemoteBinaryRoot = (
  ...
  if (!binaryPath.startsWith("/")) {
    throw new Error("junto-remote binary path must be absolute");
  }
  let real: string;
  try {
    real = realpathSync(binaryPath);
  } catch {
    throw new Error("junto-remote binary path is not resolvable");
  }
  const isWrapper = real.endsWith(RELEASE_WRAPPER_MARKER);
  const isEntry = real.endsWith(RELEASE_ENTRY_MARKER);
  if (!isWrapper && !isEntry) {
    throw new Error(
      "junto-remote is not at resources/bin/junto-remote or resources/app-remote/junto-remote.js under a release",
    );
  }
  ...
  if (!isOwnedNonLinkFile(real, isWrapper)) {
    throw new Error(
      isWrapper
        ? "junto-remote binary must be an owned non-symlink executable"
        : "junto-remote entry must be an owned non-symlink file",
    );
  }
 ...
 * `junto-remote` binary. Rejects anything outside the immutable userland layout.
 */
...
const installUnifiedCli = (release: string, home: string): void => {
  installOwnedHelper(release, home, CLI_RELATIVE, CLI_HELPER_RELATIVE, "junto");
};
...
const enableUserService = (): void => {
  ...
  const enable = spawnSync(
    "/usr/bin/systemctl",
    ["--user", "enable", "junto-remote.service"],
    { encoding: "utf8", shell: false },
  );
  if (enable.status !== 0) {
    throw new Error(
 ...
 * `binaryPath` must be the absolute path of this process's junto-remote binary.
 */
