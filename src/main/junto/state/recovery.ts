...
const STATE_BACKUP_FILE_PATTERN =
  /^junto-backup-([0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})\.db$/;
...
const inspectBackup = (
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `state backup schema is not a recognized Junto version: ${file}`,
      );
    }
 ...
 *
 * This is portability/evidence, not restore. It never replaces junto.db,
