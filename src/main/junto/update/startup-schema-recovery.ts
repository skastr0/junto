...
const SCHEMA_TOO_NEW_MESSAGE = "A newer version of Junto is required";
const SCHEMA_TOO_NEW_DETAIL = [
  "Your data was last saved by a newer version of Junto.",
  "This version cannot open it.",
  "",
  "Update to the latest Junto, then open the app again.",
].join("\n");
  ...
  try {
    ...
    if (!feedVersionUnbricks(appVersion, downloaded?.version)) {
      ...
      await ui.showMessageBox({
        ...
        detail: [
          "Your data still needs a newer Junto.",
    ...
    await ui.showMessageBox({
      ...
      detail:
        "Check your network connection and try again, or download the latest Junto from the website.",
    });
