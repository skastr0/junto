...
const ensureInstallLayout = async (home: string): Promise<string> => {
  for (const relative of [".local", ".local/opt", ".local/opt/junto-alpha", ".local/bin", ".local/share", ".local/share/applications"]) {
    await ensureOwnedDirectory(join(home, relative));
  }
  return join(home, ".local/opt/junto-alpha");
};
...
const launcherBody = (root: string, generation: string): string =>
  `#!/bin/sh\n${MANAGED_LAUNCHER}\n# generation: ${generation}\nexec ${shellLiteral(join(root, generation, "junto"))} "$@"\n`;

const launcherPath = (home: string): string => join(home, ".local/bin/junto-desktop");
const desktopPath = (home: string): string => join(home, ".local/share/applications/junto.desktop");
...
const readManagedLauncher = async (home: string, root: string): Promise<LauncherSnapshot | undefined> => {
  ...
  return { ...identity(file.stat), body: file.body, executablePath: join(root, generation, "junto") };
};
...
export const assertLinuxDesktopManagedIncumbent = async (input: { readonly home?: string; readonly executablePath: string }): Promise<void> => {
  requireOrdinaryUser();
  const home = await resolveInstallHome(input.home ?? homedir());
  for (const relative of [".local", ".local/opt", ".local/opt/junto-alpha", ".local/bin"]) await requireOwnedDirectory(join(home, relative));
  const root = join(home, ".local/opt/junto-alpha");
...
export const stageLinuxDesktopRelease = async (input: {
  ...
  return await withInstallMutation(ownedRoot, async (lease) => {
    ...
    try {
      ...
      const executable = inventory.get("junto");
      ...
      for (const relative of [".local/opt/junto-alpha", ".local/opt", ".local/bin", ".local/share/applications", ".local/share", ".local", ""]) await syncDirectory(join(home, relative));
      const handle: StagedLinuxDesktopRelease = Object.freeze({ [STAGED]: true as const, executablePath: join(generationPath, "junto"), generationPath, archiveSha256: descriptor.archive.sha256 });
...
const managedGenerationFromLauncher = async (home: string): Promise<{
  ...
  try {
    ...
    const root = join(resolvedHome, ".local/opt/junto-alpha");
