...
import { deploymentStream, homeDirectoryLookup } from "../ssh/program";
import {
...
const REMOTE_CLI_EXECUTABLE =
  `${REMOTE_APP_PATH}/Contents/Resources/bin/junto`;
...
export const admitReleaseZipArtifact = async (
  ...
  const stageRoot = await mkdtemp(join(tmpdir(), "junto-darwin-release-zip-"));
  let disposed = false;
...
const buildRemoteDeployScriptWithRuntime = (
  ...
  const remoteCliExecutablePath =
    `${remoteAppPath}/Contents/Resources/bin/junto`;
...
<key>StandardOutPath</key><string>${xmlText(logDir)}/junto.out.log</string>
<key>StandardErrorPath</key><string>${xmlText(logDir)}/junto.err.log</string>
</dict></plist>
`;
...
IN_CLI_EXE=${shellLiteral(`${incomingPath}/${APP_BUNDLE_NAME}/Contents/Resources/bin/junto`)}
