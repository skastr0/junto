...
#
#   scripts/build-app.sh              typecheck + electron-vite + electron-builder --dir
#   scripts/build-app.sh --fast       skip typecheck (package only; still compiles)
#   scripts/build-app.sh --verify     typecheck + unit tests + package
#   scripts/build-app.sh --compile-only   electron-vite only (out/), no .app
#
...
if [[ "$COMPILE_ONLY" -eq 1 ]]; then
  log "compile-only done (out/). Skip packaging."
  exit 0
fi
