...
import { resolveVellumHome } from "../src/shared/vellum-home";
import { parseContentHelperArgs } from "../src/main/vellum/content/helper-contract";
import { contentStoreRoot } from "../src/main/vellum/content/paths";
import {
...
const main = async (): Promise<void> => {
  const parsed = parseContentHelperArgs(process.argv.slice(2));
  if ("exitCode" in parsed) {
    process.stderr.write(`vellum-content: ${parsed.error}\n`);
  ...
  try {
    ...
    if (args.mode === "send") {
      ...
      // Stream remaining bytes only; no status on stdout.
      if (args.offset > 0) {
    ...
    // Incomplete is a soft exit so the CC can resume; not a hard failure.
    process.exitCode = 0;
  } catch (error) {
    const message =
    ...
    process.exitCode = error instanceof ContentStoreError && error.code === "corrupt" ? 2 : 1;
  }
};
