...
describe("work control systemd readiness", () => {
  it("ignores ambient Linux desktop runtime directories outside a systemd invocation", async () => {
    const root = await mkdtemp(join(tmpdir(), "vellum-work-ready-desktop-"));
    roots.push(root);
  ...
  it("publishes the exact private generation receipt for a systemd invocation", async () => {
    const root = await mkdtemp(join(tmpdir(), "vellum-work-ready-systemd-"));
    roots.push(root);
    await mkdir(join(root, "vellum-remote"));
    ...
    const receiptPath = join(root, "vellum-remote", `ready-${generation}`);
