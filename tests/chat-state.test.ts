...
// --- action layer: window.vellum mocked exactly like tests/mutations.test.ts,
...
describe("openChat", () => {
  ...
  it("degrades to a quiet error state when window.vellum is absent — never throws", async () => {
    clearWindow();
    const agentKey = freshAgentKey();
...
describe("setModel / closeChat / markRead", () => {
  ...
  it("closeChat sets status closed even when window.vellum is absent", async () => {
    clearWindow();
    const agentKey = freshAgentKey();
