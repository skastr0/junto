...
describe("Linux Remote userland assets", () => {
  ...
  it("ships a displayless unit template pinned to vellum-command-remote", async () => {
    ...
    expect(unit).toContain("ConditionFileIsExecutable=@JUNTO_RUNTIME_ROOT@/resources/bin/vellum-command-remote");
    expect(unit).toContain("ExecStart=@JUNTO_RUNTIME_ROOT@/resources/systemd/vellum-command-remote-launch");
