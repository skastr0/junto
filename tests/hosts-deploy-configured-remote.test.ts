...
import {
  ...
  packageAdmitted,
} from "../src/main/junto/hosts/deploy-configured-remote";
import type { DeployRemoteResult } from "../src/main/junto/hosts/remote-deployment";
...
describe("configured remote deploy receipts", () => {
  ...
  it("is receipt helpers, not a second Deploy apply loop", () => {
    const configuredRemote = readFileSync(
      new URL(
        "../src/main/junto/hosts/deploy-configured-remote.ts",
        import.meta.url,
      ),
      "utf8",
    );
