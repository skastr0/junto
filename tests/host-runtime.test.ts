...
import { HostOps, HostTarget } from "../src/main/junto/hosts/host-ops";
import {
  combineHostProcessPlanes,
  readRemoteTextFile,
  workAttachFromTermConnect,
  workAttachFromTokenFile,
} from "../src/main/junto/hosts/host-runtime-platform";
...
describe("HostRuntime inversion", () => {
  it("Deploy goes through reconcile, not the old ceremony from the coordinator", () => {
    const coordinator = readFileSync(
      new URL("../src/main/junto/hosts/operator-coordinator.ts", import.meta.url),
      "utf8",
    );
  ...
  it("keeps Darwin and Linux as HostOps layers, not apply loops", () => {
    ...
    const linuxOps = readFileSync(
      new URL("../src/main/junto/hosts/host-ops-linux.ts", import.meta.url),
      "utf8",
    );
    ...
    const platform = readFileSync(
      new URL("../src/main/junto/hosts/host-runtime-platform.ts", import.meta.url),
      "utf8",
    );
  ...
  it("does not keep a shared applyConfiguredRemoteGap act", () => {
    expect(() =>
      readFileSync(
        new URL("../src/main/junto/hosts/host-runtime-apply.ts", import.meta.url),
      ),
    ).toThrow();
  });
  ...
  it("does not mint onAdmitted pre-mutation receipts", () => {
    ...
    const configured = readFileSync(
      new URL("../src/main/junto/hosts/deploy-configured-remote.ts", import.meta.url),
      "utf8",
    );
