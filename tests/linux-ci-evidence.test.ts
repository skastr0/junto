...
describe("Linux CI log safety", () => {
  it("redacts workspace, runner, home, and secret material", () => {
    ...
    expect(result.output).toContain("<home>/.junto/work/token");
...
describe("Linux release artifact identity", () => {
  it("admits one target-specific userland runtime archive", () => {
    expect(() => validateLinuxReleaseArtifactNames({
      names: [
        "junto-runtime-0.1.0-linux-x64.tar.gz",
      ],
      expectedArchive: "junto-runtime-0.1.0-linux-x64.tar.gz",
    })).not.toThrow();
  });
  ...
  it("hashes required evidence without embedding absolute paths", async () => {
    ...
    const archive = `junto-runtime-${packageVersion}-linux-x64.tar.gz`;
    ...
    expect(linuxCiChecksumLines(manifest)).toContain(
      `release/junto-runtime-${packageVersion}-linux-x64.tar.gz`,
    );
