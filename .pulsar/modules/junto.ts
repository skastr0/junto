...
const REPOSITORY_NAME = "junto"
const CALIBRATION_SCOPE = "repo-local-junto"
const ARCHITECTURE_ROLE_RULE_ID = "junto.repository.architecture-role.v1"
const INTEGRATION_SIZE_RULE_ID = "junto.repository.integration-size-policy.v1"
const INTEGRATION_NESTING_RULE_ID = "junto.repository.integration-nesting-policy.v1"
const SHARED_CONTRACT_SIZE_RULE_ID = "junto.repository.shared-contract-size-policy.v1"
...
const architectureRoleRule = (
  ...
  if (
    pathHasPrefix(path, "src/main/junto/state") ||
    pathHasPrefix(path, "src/main/junto/station") ||
    pathHasPrefix(path, "src/main/junto/work") ||
    pathHasPrefix(path, "src/main/junto/kernel") ||
    pathHasPrefix(path, "src/main/junto/browser") ||
    pathHasPrefix(path, "src/main/junto/term") ||
...
export default defineProjectModule({
  ...
  processors: [
    defineProcessor({
      id: "junto-architecture-role-classifier",
      slot: "taxonomy.file-classifier",
      role: "enricher",
      priority: 20,
      fingerprint: "junto-architecture-role-classifier-v1",
    ...
    defineProcessor({
      id: "junto-integration-size-policy",
      slot: "typescript.size-policy",
      role: "factor-policy",
      priority: 20,
      fingerprint: "junto-integration-size-policy-v1",
    ...
    defineProcessor({
      id: "junto-shared-contract-size-policy",
      slot: "typescript.size-policy",
      role: "factor-policy",
      priority: 20,
      fingerprint: "junto-shared-contract-size-policy-v1",
      process: (current, context, runtime) =>
        Effect.gen(function* () {
          const role = yield* architectureRoleClassificationForFile(context, current.value.file)
    ...
    defineProcessor({
      id: "junto-integration-nesting-policy",
      slot: "typescript.nesting-policy",
      role: "factor-policy",
      priority: 20,
      fingerprint: "junto-integration-nesting-policy-v1",
      process: (current, context, runtime) =>
        Effect.gen(function* () {
          const role = yield* architectureRoleClassificationForFile(context, current.value.file)
          if (role !== "integration") return current
