...
test("mark all read empties the stack in one click", async ({ junto }) => {
  ...
  const stack = page.getByTestId("completed-task-notify");
  await expect(stack.locator(".completed-task-notify__item")).toHaveCount(3);
  await page.waitForTimeout(200);
  await stack.screenshot({ path: join(SHOTS, "20-mark-all-button.png") });

  // One click clears the plate without opening any task board.
  await page.getByTestId("completed-task-notify-mark-all").click();
  await expect(stack).toHaveCount(0);
  await expect(page.getByTestId("task-board")).toHaveCount(0);
  await page.screenshot({ path: join(SHOTS, "21-after-mark-all.png"), fullPage: false });
});
