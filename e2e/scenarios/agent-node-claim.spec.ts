...
test("an agent node shows the task its seat has claimed", async () => {
  ...
  try {
    ...
    const created = await page.evaluate(
      ([name]) => window.junto!.workTaskCreate(name, "tasks", "Display claim task on the node", { details: "Display claim task on the node" }),
      [canvas] as const,
    );
    ...
    const claimed = await page.evaluate(
      ([name, taskId, actor]) =>
        window.junto!.workTaskClaim(name, "tasks", taskId, actor),
