...
**Junto must never threaten the user's machine.**
...
capability that only Junto can mint when it owns the resource.
...
Scoped lifecycle deletes (app sockets under `~/.junto/…`, install staging,
...
impossible: Junto's own code cannot mint the authority without owning the
spawn path.

On macOS, process-table observation and the subsequent child or group signal
are not one atomic kernel operation. Junto therefore requires the original
...
operating-system TOCTOU limit; macOS does not offer Junto a pidfd-style atomic
...
- POSIX group signaling is still intentional for Junto-created detached
...
- SQLite state is protected by owner-only directory/file modes and one
  main-process connection. Junto does not claim containment from arbitrary
