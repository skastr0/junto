...
- a Junto-local, process-bound actor;
- a plugin without a local Junto installation that called a network-reachable
...
Junto has one v1 actor runtime and one agent work path:

1. The operator authors an `agent` node with a managed-terminal template.
2. Junto spawns and owns the terminal process on its placed installation.
...
A raw OS terminal, external harness process, or plugin without a local Junto
runtime has no Junto seat and no remote work-control route.
