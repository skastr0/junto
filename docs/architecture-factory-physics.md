...
| Role | Can be blocked? |
|------|-----------------|
| **Actor** | **Yes** — an actor is a worker; it is blocked when it holds claimed work it cannot proceed on and cannot abandon (a claimed task in `input-required` / `auth-required`, or a request it raised that is still pending — requests are claimed by their raiser at creation). |
...
`auth-required` item **claimed by** the direct `toNode` actor generates
**blocks** on that actor alone. Tasks are claimed by the worker that pulled
