...
| **Phase** | Is work blocked or free to proceed? | Derived edge phase (`blocks` \| `depends` \| `relates`) from criteria + live data | Recomputed; never authorial `ether.kind` input |
...
criteria + live data ──► phase (blocks | depends | relates)
...
| **Sink** | `task`, `page`, `project`, work surfaces | Receives ops; target of inbound capability |
| **Scheduler** | watchers / timers (kernel structures) | Pulses regions; does not hold user-facing seats |
| **Region** | group + `ether.region` | Geography + pulse briefing container |
| **Furniture** | notes, labels, pure text without live entity | Spatial annotation; no seat, no ocap wield |
...
factory humming. The only stoppages are: an actor's claimed task escalated to
`input-required` / `auth-required` (blocks that worker), and an open **request**
(always blocking). Both land on the **actor**, derived from what it holds — never
fanned red out of a sink onto its connected nodes.
...
| Modes (criteria) | — | `glyphs`, `wip`, `tasks` (see document contract) |
...
| `activity_blocked` | Bound but phase plane says blocked (criteria / deps) |
