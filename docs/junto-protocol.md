...
**Audience:** Junto contributors, reviewers, and operators qualifying
...
Junto is one operator-owned factory with one Command Center and any number of
...
Junto exposes exactly three version facts:
...
Likewise, literals such as `junto/station-protocol-preface/v1` and
...
1. one `~/.junto/state/junto.db` per installation;
...
One operator's Junto system: one sovereign intent, one Command Center, and
zero or more enrolled Remotes.

### Installation

One installed Junto app with one local database and one durable
...
host-local. Junto never relays page operations through Command Center or the
Station API.
...
exists. Junto does not pretend the two SQLite transactions are one distributed
transaction.
...
If the actor or Remote is stalled, Junto reports that fact. A future explicit
operator recovery operation must define one atomic authority cutover before it
can re-home active work.

Deleting an actor retires its stable seat and stops its Junto-owned runtime;
...
as stalled/orphaned lifecycle state. Junto does not silently turn actor
...
5. after the managed transport accepts the prompt, Junto commits an accepted
...
redeliver the same stable delivery ID. Junto must never write the receipt
...
projection. If a local resource is no longer authorized, Junto performs the
...
```text
CompatibilityOffer {
  protocol: "junto/station-protocol-preface/v1"
  frame: "offer"
...
Before Junto ships its second installed release, release qualification must
...
Command Center
  └── opens one persistent SSH command session
        └── fixed packaged junto station-stdio entry on Remote
...
2. Command Center resolves the packaged Remote platform and opens the same
   `junto station-stdio` framed SSH command used by ordinary sessions;
...
authentication. Junto does not copy or reissue those credentials.
...
surface. It is not a second credential or channel binding. Under Junto's
...
Junto must not be architected so SSH endpoints are embedded in domain
