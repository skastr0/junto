...
**Sole product store** remains `junto.db`. Install-ops / content files stay install-local (see AGENTS.md).
...
- **Never permanent:** `src/main/junto/kernel/**`, `src/main/junto/work/**` (claim/ContentService empty-Context class of bug).
...
| Product IPC | `src/main/ipc.ts`, `src/main/junto/ipc.ts` — domain Effects via `AppRuntime.runPromise` only |
| Remote boot | `src/main/junto-remote.ts` — `RemoteRuntime.runPromise` / `dispose` (audit only in S1) |
...
| Surface | Law |
|---|---|
| `src/main/junto/kernel/service.ts` | `KernelLive` captures full ambient `Effect.runtime()`; all Promise bridges use `Runtime.runPromise` — zero bare `Effect.runPromise` |
| `src/main/junto/work/service.ts` | Hard `yield* ContentService` at WorkLive build (no `serviceOption` soft-miss; missing content fails layer, not claim) |
...
| `src/main/junto/update/ipc.ts` | permanent: post-`AppRuntime.dispose` finalize only |
