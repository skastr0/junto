...
6. Among its first messages: **`junto onboard`** via the station CLI — it worked, and it injected the **task data and metadata**.
...
|---|---|---|
| PTY ownership, spawn, byte journal, resize, exit, sealed kill plane | **built** | `src/main/junto/term/local-host.ts` (single data hook at `observeData:698`), `plane.ts`, `router.ts`, `sessions.ts`, `release-fence.ts`, `shell-policy.ts` |
...
| Work-control server: all ops + scopes + process-bind identity | **built** | `src/main/junto/work/{control,authz,caller-resolve,live-seat,service}.ts` — ops include `ping doctor capabilities onboard tasks.create tasks.list tasks.show tasks.rules tasks.check tasks.claim tasks.update msg.list msg.send signal.raise signal.clear signal.list artifact.publish` |
| **Station CLI, agent-native** | **built** | `src/cli/` — `junto onboard \| doctor \| capabilities \| tasks list\|claim\|update \| msg list\|send \| escalate\|blocked\|feedback \| signal list\|clear \| artifact publish \| browser \| schema \| examples`; JSON-in/JSON-out, batch-capable, `dist/junto` via `bun run cli:build`; browser commands retain their host-local control socket behind the one agent-facing command |
...
- New `src/main/junto/term/observer/`: one headless terminal per live session, fed from `observeData:698` (the single insertion point — every byte already flows through it with a seq).
...
- Inject `PATH` so `dist/junto` resolves, including its canonical `junto browser` dispatcher; inject seat/socket/token env (verified to reach agent shell subprocesses on the original four; Prime Agent receives them through its isolated binding daemon).
...
- **`junto escalate` / `blocked` / `feedback`** — universal agent signals (no edge): the seat declares it needs the operator, the operator answers through operator mail, and nothing is enforced on the seat. They replaced the retired `request.escalate` op and its blocked-seat stop directive.
...
- **`msg.*` revived for agent-to-agent mail** — agents never touch another PTY. An agent sends via the work plane; the **kernel** delivers by typed injection whether the recipient is idle or mid-turn, tagged `mail from <seat>`; the harness queues or steers mail typed during a turn by its own rules, and Junto adds no wait or interrupt of its own. The only holds are screen guards: an operator draft, a dialog, or an unreadable composer is never typed into. Edges gate who may mail whom; mail pending for one seat goes out batched as one notice.
- **Ergonomics for a typing agent:** ops are typed by an LLM into a TUI, so error messages must be instructive and self-correcting, and `junto schema`/`examples` must cover every op. Keep JSON-in/JSON-out (already batch-capable).
- **Broadcast** — same mailbox, N seats, delivered per seat as it arrives (a working seat's harness queues it). Shift-click → type → broadcast.
...
| | Claude Code | Codex | Grok | Hermes | Prime Agent 0.9.4 |
|---|---|---|---|---|---|
