...
Junto's Linux Station lane is an ordinary-user install and update path. A
supported host does not need a custom image and Junto does not become a host
administrator. Junto may inspect the host read-only, explain a missing
...
- one signed, versioned Junto payload is installed and activated entirely
  inside the Station user's account;
- first install and later update use the same userland transaction and state
  preflight;
- Junto never invokes `sudo`, accepts an administrator password, writes
...
- host-administrator work is optional, explicit, minimal, and performed
  outside Junto;
- a missing optional host capability degrades only the affected Junto
  capability when that is safe;
- a missing security prerequisite fails the affected security boundary
  closed;
- a future Junto-hosted machine follows the same contract; it merely arrives
  with the optional host preparation already completed.

Current `.deb`, `/opt/Junto`, privileged release-installer,
...
- the Station user's identity, home ownership, writable user directories,
  ...
  install location;
- the user service manager and whether the Junto user service can run for the
  current login;
...
|---|---|---|
| `available` | The observed host facts satisfy this capability. | The capability may run, subject to normal Junto intent and authorization. |
| `degraded` | The capability is optional and unavailable without weakening a security boundary. | Junto disables or omits only that capability and reports the limit. |
| `requires-admin` | A bounded external administrator action could make the capability available. | Junto shows the action and consequence but does not execute it or collect its credentials. |
...
| `unknown` | The fact could not be observed reliably. | Junto does not infer success; the affected gate remains unavailable or blocked according to its safety requirement. |
...
2. open Doctor immediately and map the blocked capability to a documented
   remediation
   action;
3. execute only externally reviewed host actions outside Junto;
...
2. the affected Junto capability;
...
Junto may render a copyable command only when the signed release documentation
...
declare and qualify its exact display contract separately. Junto still must
...
- when the qualified path uses an AppArmor profile, an administrator may
  install or load only the exact release-reviewed profile outside Junto;
- when the qualified path uses the distribution's unprivileged user-namespace
  behavior, Junto verifies that behavior as the Station user;
- a present but broken security path is not silently reclassified as another
  path;
- Junto never disables AppArmor, changes a host-wide user-namespace sysctl,
...
continue without an interactive login; it does not make Junto more
authorized.
...
To revoke the change, first stop and disable the Junto user service as the
...
Junto reports lingering as `available`, `requires-admin`, or
...
package manager outside Junto.

Junto does not call `apt`, `apt-get`, `dpkg`, or another system package
...
application uses it. After removal, rerun Junto preflight and Doctor and
...
3. start only the affected Junto capability;
...
Removal is the inverse administrator action, performed outside Junto. Junto
...
- install a `sudoers` rule, setuid helper, file capability, polkit rule,
  ...
  mutation bridge for Junto updates;
...
- treat SSH reachability, package-manager success, a stale receipt, or a
  prepared hosted image as proof that Junto is ready;
