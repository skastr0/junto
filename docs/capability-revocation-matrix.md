...
|----------|--------|---------|--------|----------|
| Identity source | Unix peer PID (+ ancestor walk) of a main-registered process | Work + browser control call `admitProcessIdentity` → `readUnixPeerPid` + `resolveInTree` | **Met** | `src/main/junto/process-identity.ts`, `work/control.ts` (~901–923), `browser/edge-grant.ts` `admitSocket` |
...
| Concern | Path |
|---------|------|
| Process-bind map + peer PID | `src/main/junto/process-identity.ts` |
| Work socket + per-op admit | `src/main/junto/work/control.ts`, `work/authz.ts`, `work/caller-resolve.ts` |
| Pure physics admit | `src/shared/physics/admit.ts`, `kinds.ts`, `view.ts`, `work-ports.ts` |
| Browser process-bind + edge-grant | `src/main/junto/browser/process-bind.ts`, `edge-grant.ts`, `authz.ts` |
| Browser capability leases | `src/main/junto/browser/capabilities.ts` |
...
| OwnedProcess signals | `src/main/junto/process-signal.ts`, `app-process-plane.ts` |
| UI capability chips | `src/renderer/components/InspectorFields.tsx` |
| Chat bind/unbind | `src/main/junto/chat/service.ts` |
