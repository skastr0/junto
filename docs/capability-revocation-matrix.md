...
| Client nodeRef | Never identity | Token + peer PID only; `VELLUM_NODE_REF` is not an identity claim on control | **Met** | `work/control.ts` comments; `cli/core/socket.ts`; env still named for other tooling, not admit |
...
| Transport token | Shared secret for socket membership, not principal | Bearer token at `~/.vellum/work/token` (work) / browser control token; checked before process-bind | **Met** | `work/control.ts` `workTokenMatches`; not a substitute for PID |
...
- **Target:** revoke admission → graceful terminate OwnedProcess → verify exit
  (security doctrine tier 1/2 Vellum-owned).
...
- **Slice:** treat herdr as often **externally attached** (doctrine tier
  language): revoke Vellum capability always; kill only on explicit policy or
...
4. Remote agents: close Vellum session; report if provider process is external.
